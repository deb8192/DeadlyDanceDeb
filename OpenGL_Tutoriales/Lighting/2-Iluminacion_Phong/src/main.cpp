...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))  //Comprueba al inicio de la iteracion si se ha ordenado cerrar GLFW, si es asi es TRUE y sale del bucle
    {
        ...
        ourShader->setVec3("lightPos",lightPos);               //Enviar la posicion de la luz al fragment para calcular la luz difusa
        ourShader->setVec3("viewPos", camera->Position);       //Enviar la posicion de la camara al fragment para calcular la luz especular
