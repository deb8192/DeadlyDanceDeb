...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))  //Comprueba al inicio de la iteracion si se ha ordenado cerrar GLFW, si es asi es TRUE y sale del bucle
    {
        ...
        //Posiciones
        ourShader->setVec3("light.position", camera->Position);    //Enviar la posicion de la luz al fragment
        ourShader->setVec3("light.direction", camera->Front);      //Enviar la direccion de la luz al fragment
        ourShader->setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));      //Punto de corte
        ourShader->setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f))); //Punto de corte con el cono externo
