...
int main()
{
    ...
    glm::vec3 cubePositions[] = {     //Posiciones de 10 cubos
    ...
    while(!glfwWindowShouldClose(window))  //Comprueba al inicio de la iteracion si se ha ordenado cerrar GLFW, si es asi es TRUE y sale del bucle
    {
        ...
        //Comandos de render
        glClearColor(0.1f, 0.1f, 0.1f, 1.0f);                //Designamos a el glClearColor un color
        ...
        //Propiedades del material en el caso que no tenga textura y mapa de brillos (http://devernay.free.fr/cours/opengl/materials.html)
        ...
        ourShader->setVec3("dirLight.direction", -0.2f, -1.0f, -0.3f); //Enviar la direccion donde mira la luz direccional
        ...
        //Foco de luz
        ourShader->setVec3("spotLight.position", camera->Position);    //Enviar la posicion de la camara
        ourShader->setVec3("spotLight.direction", camera->Front);      //Enviar la direccion de la camara
        ...
        ourShader->setFloat("spotLight.cutOff", glm::cos(glm::radians(12.5f)));       //Punto de corte
        ourShader->setFloat("spotLight.outerCutOff", glm::cos(glm::radians(15.0f)));  //Punto de corte con el cono externo
        ...
        //Camara para cubo de luz
        ourShaderLight->use();
        ourShaderLight->setMat4("projection", projection);
        ...
        for (unsigned int i = 0; i < 4; i++)
        {
             ...
             ourShaderLight->setMat4("model", model);
             glDrawArrays(GL_TRIANGLES, 0, 36);
        }
